(() => {
  ...
  frappe.ui.form.Layout = class Layout {
    ...
    show_message(html, color, permanent = false) {
      ...
      const block_color = color && ["yellow", "blue", "red", "green", "orange"].includes(color) ? color : "blue";
    ...
    set_tab_as_active() {
      ...
      const tab = this.tabs.find((tab2) => tab2.df.fieldname === tab_from_hash);
      if (tab) {
        tab.set_active();
        return;
      }
      ...
      if (frm_active_tab) {
        ...
        let first_visible_tab = this.tabs.find((tab2) => !tab2.is_hidden());
    ...
    make() {
      ...
      if (this.fields) {
        ...
        $.each(this.fields_list, function(i, field) {
          ...
          if (def_value == null || !def_value && !frappe.model.is_numeric_field(field.df.fieldtype))
            return;
        ...
        function(results) {
          ...
          if (results.length) {
            for (const v of results) {
              var row = $(
                repl(
                  '<div class="row link-select-row">						<div class="col-xs-4 ellipsis">							<b><a href="#">%(name)s</a></b></div>						<div class="col-xs-8">							<span class="text-muted">%(values)s</span></div>						</div>',
                  {
                    name: v[0],
  ...
  frappe.ui.form.MultiSelectDialog = class MultiSelectDialog {
    ...
    init() {
      ...
      this.make();
      this.selected_fields = /* @__PURE__ */ new Set();
    }
    get_fields() {
      ...
      if ($.isArray(this.setters)) {
        ...
        Object.keys(this.setters).forEach((setter, index) => {
          ...
          columns[(index + 1) % 3].push({
            ...
            read_only: (this == null ? void 0 : this.read_only_setters) && this.read_only_setters.includes(setter) || 0,
    ...
    empty_list() {
      let checked = this.results.filter((result) => this.selected_fields.has(result.name)).map((item) => __spreadProps(__spreadValues({}, item), {
        checked: true
      }));
...
//# sourceMappingURL=dialog.bundle.7AG42GA6.js.map
