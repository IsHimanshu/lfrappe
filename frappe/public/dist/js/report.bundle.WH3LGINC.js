(() => {
  ...
  var require_frappe_datatable_cjs = __commonJS({
    "node_modules/frappe-datatable/dist/frappe-datatable.cjs.js"(exports, module) {
      ...
      var DataManager = class {
        ...
        prepareRow(row, meta) {
          row = row.map((cell, i) => this.prepareCell(cell, i)).map((cell) => {
      ...
      var CellManager = class {
        ...
        bindMouseEvents() {
          ...
          if (this.options.showTotalRow) {
            $2.on(this.footer, "click", ".dt-cell", (e) => {
              this.focusCell($2(e.delegatedTarget));
            });
          }
          const selectArea = (e) => {
        ...
        getEditor(colIndex, rowIndex, value, parent2) {
          const column = this.datamanager.getColumn(colIndex);
          const row = this.datamanager.getRow(rowIndex);
          const data = this.datamanager.getData(rowIndex);
          let editor = this.options.getEditor ? this.options.getEditor(colIndex, rowIndex, value, parent2, column, row, data) : this.getDefaultEditor(parent2);
          if (editor === false) {
            return false;
          }
          if (editor === void 0) {
            editor = this.getDefaultEditor(parent2);
          }
          return editor;
        }
        getDefaultEditor(parent2) {
        ...
        scrollToCell($cell) {
          if ($2.inViewport($cell, this.bodyScrollable) || $2.inViewport($cell, this.footer))
            return false;
          const {
            rowIndex
          } = $2.data($cell);
  ...
  frappe.views.ReportView = class ReportView extends frappe.views.ListView {
    ...
    setup_defaults() {
      ...
      if (this.report_name) {
        return this.get_report_doc().then((doc) => {
          ...
          this.filters = [
            ...Array.isArray(this.report_doc.json.filters) ? this.report_doc.json.filters : [],
    ...
    setup_sort_selector() {
      ...
      if ((_b = (_a = this.report_doc) == null ? void 0 : _a.json) == null ? void 0 : _b.group_by) {
        this.group_by_control.apply_settings(this.report_doc.json.group_by);
      } else if ((_c = this.view_user_settings) == null ? void 0 : _c.group_by) {
    ...
    get_count_element() {
      ...
      if (!$count.length) {
        $count = $("<span>").addClass("text-muted list-count").prependTo(this.$paging_area.find(".level-right"));
      }
      return $count;
    }
    on_update(data) {
    ...
    get_editing_object(colIndex, rowIndex, value, parent2) {
      const control = this.render_editing_input(colIndex, value, parent2);
      if (!control)
        return false;
    ...
    render_editing_input(colIndex, value, parent2) {
    ...
    is_editable(df, data) {
      if (df && frappe.model.can_write(this.doctype) && (data.docstatus !== 1 || df.allow_on_submit) && data.docstatus !== 2 && !df.read_only && !df.is_virtual && !df.hidden && frappe.model.is_non_std_field(df.fieldname)) {
        return this.meta.fields.some((df2) => df2.fieldtype === "Table") || df.read_only_depends_on && !this.evaluate_read_only_depends_on(df.read_only_depends_on, data);
      }
      return false;
    }
    get_data(values) {
      ...
      table_fields.forEach((df) => {
        ...
        dialog_fields.push({
          ...
          options: columns[cdt].filter((df2) => {
            ...
            label: __(df2.label, null, df2.parent),
            value: df2.fieldname,
          ...
          if (Array.isArray(row)) {
            doc = row.reduce((acc, curr) => {
              if (!curr.column.docfield)
                return acc;
              if (curr.column.docfield.fieldtype == "Link" && frappe.boot.link_title_doctypes.includes(
                curr.column.docfield.options
              ) && curr.html) {
                this.link_title_doctype_fields[curr.content] = curr.column.docfield.options;
              }
  ...
  frappe.views.QueryReport = class QueryReport extends frappe.views.BaseList {
    ...
    get_report_doc() {
      return frappe.model.with_doc("Report", this.report_name).then((doc) => {
        ...
        var _a;
        return frappe.model.with_doctype((_a = this.report_doc) == null ? void 0 : _a.ref_doctype);
      });
    }
    get_report_settings() {
      return new Promise((resolve, reject) => {
    ...
    setup_progress_bar() {
      ...
      const execution_time = ((_a = this.report_settings) == null ? void 0 : _a.execution_time) || 0;
      if (execution_time < 5)
        return;
    ...
    setup_filters() {
      ...
      this.filters = filters.map((df, index) => {
    ...
    refresh(have_filters_changed) {
      ...
      return new Promise((resolve) => {
        ...
        (_b = (_a = this.report_settings).after_refresh) == null ? void 0 : _b.call(_a, this);
      });
    }
    render_summary(data) {
    ...
    render_datatable() {
      ...
      if (data.length > (cint(frappe.boot.sysdefaults.max_report_rows) || 1e5)) {
        let msg = __(
    ...
    get_chart_options(data) {
      ...
      if (options.fieldtype) {
        options.tooltipOptions = {
          formatTooltipY: (d) => frappe.format(
            d,
            {
    ...
    prepare_columns(columns) {
      let is_query_generated_report = this.report_doc.query && this.report_doc.query != void 0 && this.report_doc.query != "";
      return columns.map((column) => {
        ...
        return Object.assign(column, {
          ...
          editable: (_a = column.editable) != null ? _a : false,
          compareValue: compareFn,
    ...
    get_filter_values(raise) {
      ...
      return this.filters.filter((f) => {
        var _a;
        return (_a = f.get_value) == null ? void 0 : _a.call(f);
      }).map((f) => {
        var v = f.get_value();
        if (f.df.hidden)
          v = f.value;
    ...
    export_report() {
      ...
      this.export_dialog = frappe.report_utils.get_export_dialog(
        ...
        ({
          ...
          const visible_idx = ((_a = this.datatable) == null ? void 0 : _a.bodyRenderer.visibleRowIndices) || [];
          if (visible_idx.length + 1 === ((_b = this.data) == null ? void 0 : _b.length)) {
            visible_idx.push(visible_idx.length);
          }
          const args = {
            cmd: "frappe.desk.query_report.export_query",
            report_name: this.report_name,
            custom_columns: ((_c = this.custom_columns) == null ? void 0 : _c.length) ? this.custom_columns : [],
              ...
              fields: [
                {
                  ...
                  options: (_a = this.linked_doctypes) == null ? void 0 : _a.map((df) => ({
  ...
  // frappe-html:/home/frappe/frappe-bench/apps/frappe/frappe/public/js/frappe/views/reports/print_grid.html
...
`;
  ...
  // frappe-html:/home/frappe/frappe-bench/apps/frappe/frappe/public/js/frappe/views/reports/print_tree.html
...
`;
  ...
  // frappe-html:/home/frappe/frappe-bench/apps/frappe/frappe/public/js/frappe/ui/group_by/group_by.html
...
`;
  ...
  frappe.ui.GroupBy = class {
          ...
          for (let doctype in this.all_fields) {
            ...
            doctype_fields.forEach((field) => {
              if (frappe.model.is_numeric_field(field.fieldtype)) {
                ...
                let option_text = doctype == this.doctype ? __(field_label, null, field.parent) : `${__(field_label, null, field.parent)} (${__(doctype)})`;
    ...
    apply_group_by() {
      if (this.group_by_doctype && this.aggregate_on_doctype && this.aggregate_on_doctype != this.doctype && this.group_by_doctype != this.aggregate_on_doctype) {
        frappe.msgprint(
    ...
    get_group_by_field_label() {
      ...
      return (field == null ? void 0 : field.label) ? __(field.label, null, field.parent) : field == null ? void 0 : field.fieldname;
    }
  };
      ...
      function update_csv_preview(dialog2) {
        ...
        dialog2.set_value(
          ...
          frappe.report_utils.get_csv_preview(
            ...
            dialog2.get_value("csv_decimal_sep")
          )
        );
      }
...
//# sourceMappingURL=report.bundle.WH3LGINC.js.map
